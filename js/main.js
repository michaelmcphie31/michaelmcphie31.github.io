...
const heroVideoPath = "assets/videos/workout-background.mp4?v=20260725-resources-blackout";
...
const revealTargets = document.querySelectorAll(
  ".section-heading, .intro-grid > *, .page-hero-grid > *, .split-copy, .image-panel, .values-grid article, .service-row, .price-card, .calendar-panel, .booking-form, .review-section-action, .testimonial-card, .submitted-testimonial-grid, .guide-card, .evidence-grid a, .resources-cta"
);
...
const coachingSchedule = [
  {
    days: [1, 2, 3, 4, 5],
    windows: [{ start: "7:00 AM", end: "7:00 PM" }],
  },
  {
    days: [6],
...
const handsOnSchedule = [
  {
    days: [1, 2, 3, 4, 5],
    windows: [{ start: "3:00 PM", end: "6:00 PM" }],
  },
  {
    days: [6],
...
const serviceConfigs = {
  "1:1 Coaching": {
    ...
    summary:
      "Available Monday through Friday from 7:00 AM to 7:00 PM CT and Saturdays from 1:00 PM to 5:00 PM CT. All appointments require at least 48 hours' notice.",
  ...
  Pretzeling: {
    ...
    summary:
      "Available Monday through Friday from 3:00 PM to 6:00 PM CT, Saturdays from 8:00 AM to 12:00 PM CT, and Sundays from 1:00 PM to 5:00 PM CT. All appointments require at least 48 hours' notice.",
...
const updateBookingCopy = () => {
  ...
  serviceChoiceButtons.forEach((button) => {
    const selected = button.getAttribute("data-service-choice") === serviceKey;
...
serviceChoiceButtons.forEach((button) => {
  button.addEventListener("click", () => {
    const service = button.getAttribute("data-service-choice");
